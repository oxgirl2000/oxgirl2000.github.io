...
const frameRateLog = false;       // mode: logs frame rate every second.
...
const containerName = '#p5-container';
...
function setup() {
  //checkOrientation();
  container = select(containerName);
  updateContainerSize();
...
/**
  Checks size of container and update containerWidth and containerHeight;
 */
function updateContainerSize() {  
  containerWidth = container.style('width');
  containerWidth = containerWidth.substring(0, containerWidth.length-2);        // slice off "px"
...
function windowResized() {  
  ...
  currentCount = totalParticleCount;
  updateContainerSize();
